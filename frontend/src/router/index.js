import { createRouter, createWebHashHistory } from 'vue-router'

const routes = [
  { path: '/login', name: 'Login', component: () => import('../views/Login.vue'), meta: { public: true } },
  {
    path: '/',
    component: () => import('../views/Layout.vue'),
    redirect: '/dashboard',
    children: [
      { path: 'dashboard', name: 'Dashboard', component: () => import('../views/Dashboard.vue'), meta: { title: '数据大盘' } },
      { path: 'keywords', name: 'Keywords', component: () => import('../views/Keywords.vue'), meta: { title: '关键词管理' } },
      { path: 'tasks', name: 'Tasks', component: () => import('../views/Tasks.vue'), meta: { title: '任务中心' } },
      { path: 'industry-batch', name: 'IndustryBatch', component: () => import('../views/IndustryBatch.vue'), meta: { title: '行业批量采集' } },
      { path: 'realleads',
      name: 'RealLeads',
      component: () => import('../views/RealLeads.vue'),
      meta: { title: '真实数据' },
    },
    {
      path: 'official', name: 'OfficialApi', component: () => import('../views/OfficialApi.vue'), meta: { title: '官方API采集' } },
      { path: 'leads', name: 'Leads', component: () => import('../views/Leads.vue'), meta: { title: '线索库' } },
      { path: 'heatmap', name: 'Heatmap', component: () => import('../views/Heatmap.vue'), meta: { title: '地域热力导航' } },
      { path: 'analysis', name: 'AIAnalysis', component: () => import('../views/AIAnalysis.vue'), meta: { title: 'AI 分析' } },
      { path: 'member', name: 'MemberCenter', component: () => import('../views/MemberCenter.vue'), meta: { title: '会员中心' } },
      { path: 'system', name: 'SystemAdmin', component: () => import('../views/SystemAdmin.vue'), meta: { title: '系统管理' } },
      { path: 'dev', name: 'DeveloperOptions', component: () => import('../views/DeveloperOptions.vue'), meta: { title: '开发者选项', requiresDev: true } },
      { path: 'devadmin', name: 'DeveloperAdmin', component: () => import('../views/DeveloperAdmin.vue'), meta: { title: '开发者总后台', requiresDev: true } },
      { path: 'promotion', name: 'Promotion', component: () => import('../views/Promotion.vue'), meta: { title: '推广活动' } },
      { path: 'disclaimer', name: 'Disclaimer', component: () => import('../views/Disclaimer.vue'), meta: { title: '合规声明' } }
    ]
  }
]

const router = createRouter({
  // GitHub Pages 不支持 SPA History 回退，必须用 Hash 模式：刷新/直达子路由不 404
  history: createWebHashHistory(),
  routes
})

router.beforeEach((to, from, next) => {
  const token = localStorage.getItem('token')
  if (!to.meta.public && !token) {
    next('/login')
    return
  }
  // 开发者选项：仅开发者账号可访问
  if (to.meta.requiresDev) {
    const u = JSON.parse(localStorage.getItem('user') || '{}')
    if (u.username !== 'admin' && u.role_type !== 'admin' && !u.is_developer) {
      next('/dashboard')
      return
    }
  }
  next()
})

export default router
