<template>
  <div class="real-leads">
    <div class="page-header">
      ...
      <div class="actions">
        <el-input
          ...
          @keyup.enter="load"
        />
        <el-select v-model="industry" placeholder="行业筛选" clearable style="width: 160px; margin-right: 10px">
        ...
        <el-select v-model="custFilter" placeholder="客户筛选" clearable style="width: 130px; margin-right: 10px">
          <el-option label="✅ 仅看客户" value="customer" />
          <el-option label="❌ 仅看非客户" value="not_customer" />
          <el-option label="⏳ 未分类" value="unclassified" />
        </el-select>
        <el-button type="primary" :loading="importing" @click="doImport">导入 MediaCrawler 数据</el-button>
    ...
    <el-table :data="rows" v-loading="loading" border stripe>
      <el-table-column prop="intent_score" label="意向分" width="70" sortable>
      ...
      <el-table-column prop="title" label="内容" min-width="280" show-overflow-tooltip />
      ...
      <el-table-column prop="needs" label="客户需求" width="140" show-overflow-tooltip>
        <template #default="{ row }">
          <span :class="{ 'need-text': row.is_customer }">{{ row.needs || '—' }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="like_count" label="点赞" width="70" sortable />
      <el-table-column prop="comment_count" label="评论" width="70" sortable />
      <el-table-column prop="share_count" label="转发" width="70" sortable />
      <el-table-column prop="publish_time" label="发布时间" width="150" />
      <el-table-column label="原文" width="80">
        <template #default="{ row }">
          <el-link type="primary" :href="row.url" target="_blank" :underline="false">查看</el-link>
        </template>
      </el-table-column>
    </el-table>
